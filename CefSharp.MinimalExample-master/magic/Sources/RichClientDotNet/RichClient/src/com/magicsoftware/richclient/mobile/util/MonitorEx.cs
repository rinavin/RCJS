...
//
//    3.10.2008 added Wait(int millisecondsTimeout, bool exitContext) (yoelp)
//==========================================================================================
using System;
...
namespace com.magicsoftware.richclient.mobile.util
{
	...
	internal sealed class MonitorEx
	{
		...
		/// which point it will have the same lock count as it had before), or 
		/// timeout has elapsed.
		/// </summary>
      internal bool Wait(int millisecondsTimeout, bool exitContext)
		{
			int oldLockCount;
			lock (stateLock)
			{
         ...
         waitPulseEvent.WaitOne(millisecondsTimeout, false);
         if (TryEnter())
         {
	...
	internal class SynchronizationLockException : SystemException
	{
		...
		internal SynchronizationLockException(string message) : 
			base(message)
		{
		}
	}
