...
namespace com.magicsoftware.richclient.env
{
   ...
   internal class MirrorString : IMirrorXML
   {
      private Boolean _reserved; // Is this variable reserved - can not be changed by the user
      ...
      ///   CTOR
      /// </summary>
      internal MirrorString(String s)
      {
         _value = s;
         _reserved = false;
      }
      ...
      {
         return ConstInterface.MG_ATTR_ENV_VALUE + "=\"" + XmlParser.escape(_value) + "\" " +
      ...
      {
         int valueStart, valueEnd, reserveStart, paramEnd;
         String xmlData = xmlParser.getXMLdata();
         String valueAttr = ConstInterface.MG_ATTR_ENV_VALUE + "=\"";
         ...
         valueStart = xmlData.IndexOf(valueAttr, xmlParser.getCurrIndex(), paramEnd - xmlParser.getCurrIndex());
         if (valueStart != -1)
         {
            valueStart += valueAttr.Length;
            reserveStart = xmlData.IndexOf(ConstInterface.MG_ATTR_ENV_RESERVED + "=\"", xmlParser.getCurrIndex(),
            ...
            _value = xmlData.Substring(valueStart, valueEnd - valueStart).Trim();
            if (reserveStart != -1)
            {
               reserveStart += ConstInterface.MG_ATTR_ENV_RESERVED.Length + 2; // skip ="
               _reserved = (xmlData[reserveStart] == 'Y');
            }
            else
               _reserved = false;
         ...
         else if (xmlData.IndexOf(ConstInterface.MG_ATTR_ENV_REMOVED, xmlParser.getCurrIndex(), paramEnd - xmlParser.getCurrIndex()) != -1)
         {
   ...
   /// </summary>
   internal class EnvParamsTable : MirrorPrmMap<MirrorString>
   {
      /// <summary>
      ///   CTOR
      /// </summary>
      internal EnvParamsTable()
      {
         mirroredID = ConstInterface.MG_TAG_ENV_PARAM;
      }
      ...
      internal new String get(string s)
      {
         String valName = null;
         String ret = null;
         ...
         {
            valName = s;
            s = s.Insert(0, ConstInterface.INI_SECTION_MAGIC_ENV_BRACKETS);
         }
            ...
            valName = s.Substring(ConstInterface.INI_SECTION_MAGIC_ENV_BRACKETS.Length);
         ...
         if (valName != null)
            ret = getFromEnvironment(valName);
      ...
      /// <returns></returns>
      internal String getln(String section, int number)
      {
         ...
         foreach (String key in values.Keys)
         {
            if (key.StartsWith(section))
            {
               ...
               if (number > 1)
                  ...
                  return key.Substring(section.Length) + "=" + values[key];
            }
      ...
      /// <returns></returns>
      private String getFromEnvironment(String valName)
      {
         switch (valName.ToLower())
         {
            case ConstInterface.MG_ATTR_DECIMAL_SEPARATOR:
               return ClientManager.Instance.getEnvironment().GetDecimal().ToString();
            case ConstInterface.MG_ATTR_DATE:
               return DisplayConvertor.Instance.getDateChar().ToString();
            case ConstInterface.MG_ATTR_TIME:
               return ClientManager.Instance.getEnvironment().GetTime().ToString();
            ...
            case ConstInterface.MG_ATTR_DATEMODE:
               return ClientManager.Instance.getEnvironment().GetDateMode(ClientManager.Instance.getCurrTask().getCompIdx()).ToString();
      ...
      /// <param name = "valName"></param>
      /// <returns></returns>
      private String getProcessedValue(String valName, String val)
      {
      ...
      private bool isReservedLogicalName(string name)
      {
         ...
         if (!name.StartsWith("["))
         {
            name = name.Insert(0, ConstInterface.INI_SECTION_LOGICAL_NAMES_BRACKETS);
            if (values.ContainsKey(name) && values[name].isReserved())
               return true;
         }
         return false;
      }
      ...
      /// <returns></returns>
      private int getAllowedSeparators(String name)
      {
         if (name[0] != '[')
            return 0;

         String section = name.Substring(1, name.IndexOf(']') - 1);
         String param = name.Substring(name.IndexOf(']') + 1);

         switch (section.ToUpper())
         {
            case ConstInterface.INI_SECTION_MAGIC_ENV:
               if (param.Equals(ConstInterface.MG_ATTR_LANGUAGE, StringComparison.OrdinalIgnoreCase))
                  return 999;
               else
                  return 0;
      ...
      private void checkSetInEnvironment(String name, String val)
      {
         if (name.StartsWith(ConstInterface.INI_SECTION_MAGIC_ENV_BRACKETS))
         {
            name = name.Substring(ConstInterface.INI_SECTION_MAGIC_ENV_BRACKETS.Length);
            ...
            switch (name.ToLower())
            {
               case ConstInterface.MG_ATTR_DECIMAL_SEPARATOR:
                  ...
                  break;
               case ConstInterface.MG_ATTR_DATE:
                  DisplayConvertor.Instance.setDateChar(val[0]);
                  break;
               case ConstInterface.MG_ATTR_TIME:
               ...
               case ConstInterface.MG_ATTR_DATEMODE:
                  ...
                  break;
               case ConstInterface.MG_ATTR_CENTURY:
                  ClientManager.Instance.getEnvironment().setCentury(ClientManager.Instance.getCurrTask().getCompIdx(), Int32.Parse(val));
                  break;
               case ConstInterface.MG_ATTR_IDLETIME:
                  ClientManager.Instance.getEnvironment().setIdleTime(ClientManager.Instance.getCurrTask().getCompIdx(), Int32.Parse(val));
                  break;
               ...
               case ConstInterface.MG_ATTR_CONTEXT_INACTIVITY_TIMEOUT_LOWER:
                  ClientManager.Instance.getEnvironment().setContextInactivityTimeout(long.Parse(val));
                  break;
               case ConstInterface.MG_ATTR_CONTEXT_UNLOAD_TIMEOUT_LOWER:
                  ClientManager.Instance.getEnvironment().setContextUnloadTimeout(long.Parse(val));
                  break;
               case ConstInterface.MG_ATTR_TERMINAL:
                  ClientManager.Instance.getEnvironment().setTerminal(Int32.Parse(val));
                  break;
               case ConstInterface.MG_ATTR_CLOSE_TASKS_ON_PARENT_ACTIVATE:
      ...
      internal String translate(String logicalName)
      {
         StringBuilder trans = new StringBuilder(logicalName);
      ...
      private void log(StringBuilder stringBldr)
      {
         int offset; // a single cell array to be used as [in/out] argument
         int oldLen;
         bool finished;
         ...
         do
         {
            offset = 0;
            finished = true;
            while (offset < stringBldr.Length && stringBldr[offset] != '\x0000')
            {
               // '$' was define the logical name, not used any more
               if (stringBldr[offset] == '%')
               {
                  oldLen = stringBldr.Length;
                  offset = log_trans(stringBldr, offset);
                  if (oldLen != stringBldr.Length)
                     finished = false;
               }
               else
                  offset++;
      ...
      private int log_trans(StringBuilder stringBldr, int offset)
      {
         ...
         else if (stringBldr[endSym] != '%')
            offset++;

         // if (searchInSecretNames)
         // TODO: when support secret names , need to to it here
         // need also to search reserved(internal) names in logical table ONLY
         // if ((sym.equals(STR_ENGINEDIR) || sym.equals(STR_WORKINGDIR) || sym.equals(STR_WORKINGDIR)))
         // TODO: if not found in param table then search in logical name table
         if (!ignoreTrans)
         {
            String sym = ConstInterface.INI_SECTION_LOGICAL_NAMES_BRACKETS +
                         stringBldr.ToString(startSym + 1, endSym - startSym - 1);
            String trans = get(sym);
