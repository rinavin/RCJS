...
namespace com.magicsoftware.controls.designers
{
   ...
   public class FormDocumentDesigner : ScrollableControlDesigner
   {

 #region original FormDocumentDesigner
     ...
     private bool inAutoscale = false;
      ...
      int minimumGridSize = 2;
      int maximumGridSize = 200;
      ...
      protected override bool DrawGrid
      {
         get
         {
            IShowGridProperty form = Component as IShowGridProperty;
            bool canShowGrid = false;

            if (form != null)
            {
               canShowGrid = form.ShowGrid;
            }
...
#pragma warning restore 618
      ...
      private double Opacity
      {
         ...
         set
         {
            ...
            if (value < 0.0f || value > 1.0f)
            {
               ...
               throw new ArgumentException("InvalidBoundArgument",
      ...
      {
         if (gridSize.Width < minimumGridSize)
            gridSize.Width = minimumGridSize;
         if (gridSize.Width > maximumGridSize)
            gridSize.Width = maximumGridSize;

         if (gridSize.Height < minimumGridSize)
            gridSize.Height = minimumGridSize;
         if (gridSize.Height > maximumGridSize)
            gridSize.Height = maximumGridSize;
         ...
         get
         {
            ...
            if (Control.Padding == Padding.Empty && snapLines != null)
            {
               ...
               for (int i = 0; i < snapLines.Count; i++)
               {
                  ...
                  if (snapLine != null && snapLine.Filter != null && snapLine.Filter.StartsWith(SnapLineConsts.Padding))
                  {

                     if (snapLine.Filter.Equals(SnapLineConsts.PaddingLeft) || snapLine.Filter.Equals(SnapLineConsts.PaddingTop))
                     {
                     ...
                     if (snapLine.Filter.Equals(SnapLineConsts.PaddingRight) || snapLine.Filter.Equals(SnapLineConsts.PaddingBottom))
                     {
...
#pragma warning restore 618
      ...
      public sealed class SnapLineConsts
      {
...
#region CanParent
      ...
      /// <summary>
      /// handle the drag drop of other controls on Form control
      /// </summary>
      /// <param name="de"></param>
      ...
      public override SelectionRules SelectionRules
      {
         get
         {
            // Don't allow selection of IO forms
            if (Control.Parent.Parent != null && Control.Parent.Parent is EnhancedSplitContainer)
