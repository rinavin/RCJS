...
namespace com.magicsoftware.unipaas.management.gui
{
   ...
   {
      private readonly Hashtable _actionKeysMap;
      ...
      {
         internal int InternalEventCode { get; private set; }
         internal StringBuilder CtrlText { get; set; }
         ...
         internal EditParams(int startPos, int endPos, int internalEventCode)
         {
            StartPos = startPos;
            EndPos = endPos;
            InternalEventCode = internalEventCode;
            CtrlText = null;
         }
      ...
      private void setKeyBinding(int action, String keys)
      {
         _actionKeysMap[action] = keys;
      }
      ...
      /// <param name = "action"></param>
      /// <returns></returns>
      private String getKeyFromAct(int action)
      {
         var keys = (String) _actionKeysMap[action];
         return keys;
      }
      ...
      {
         setKeyBinding(InternalInterface.MG_ACT_EDT_NXTCHAR, "{RIGHT}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_PRVCHAR, "{LEFT}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_PRVLINE, "{UP}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_NXTLINE, "{DOWN}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_DELCURCH, "{DELETE}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_DELPRVCH, "{BACKSPACE}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_PRVPAGE, "{PGUP}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_NXTPAGE, "{PGDN}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_BEGNXTLINE, "{ENTER}");
         setKeyBinding(InternalInterface.MG_ACT_CLIP_COPY, "^(C)");
         setKeyBinding(InternalInterface.MG_ACT_CLIP_PASTE, "^(V)");
         setKeyBinding(InternalInterface.MG_ACT_EDT_PRVWORD, "^{LEFT}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_NXTWORD, "^{RIGHT}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_BEGLINE, "{HOME}");
         setKeyBinding(InternalInterface.MG_ACT_EDT_ENDLINE, "{END}");
      }
      ...
      /// <returns></returns>
      public EditReturnCode ProcessAction(MgControlBase ctrl, int internalEventCode, String value, ImeParam imeParam)
      {
         return ProcessAction(ctrl, internalEventCode, value, -1, -1, imeParam);
      }

      public EditReturnCode ProcessAction(MgControlBase ctrl, int internalEventCode, String value, int start, int end, ImeParam imeParam)
      {
         EditReturnCode ret = EditReturnCode.EDIT_CONTINUE;
         ...
         if (ctrl != null)
         {
            ...
            if (internalEventCode == InternalInterface.MG_ACT_CHAR && UtilStrByteMode.isLocaleDefLangKOR() && 
                ...
                UtilStrByteMode.isKoreanCharacter(value[0]))
            {
            ...
            else
            {
               ...
               MgPoint point = Manager.SelectionGet(ctrl);
         ...
         switch (internalEventCode)
         {
            case InternalInterface.MG_ACT_CHAR:
               {
                  ImeParam im;
                  if (UtilStrByteMode.isLocaleDefLangKOR() && (im = imeParam) != null)
                     sendKoreanImeMessage(ctrl, editParams, im);
                  else
                     ret = actionChar(ctrl, editParams, value);
               }
               break;

            case InternalInterface.MG_ACT_EDT_NXTCHAR:
               ret = (ctrl.isMultiline() || ctrl.getPIC().isHebrew())
                        ? postAction(ctrl, internalEventCode)
                        : actionNextChar(ctrl, editParams);
               break;

            case InternalInterface.MG_ACT_EDT_PRVCHAR:
               ret = (ctrl.isMultiline() || ctrl.getPIC().isHebrew())
                        ? postAction(ctrl, internalEventCode)
                        : actionPrevChar(ctrl, editParams);
               break;
            ...
            case InternalInterface.MG_ACT_EDT_DELCURCH:
               {
                  ...
                  ret = ctrl.isMultiline()
                        ? postAction(ctrl, internalEventCode)
                        : actionDelChar(ctrl, editParams, true);
               }
               break;
            ...
            case InternalInterface.MG_ACT_EDT_DELPRVCH:
               {
                  ...
                  ret = ctrl.isMultiline()
                     ? postAction(ctrl, internalEventCode)
                     : actionDelChar(ctrl, editParams, false);
               }
               break;
            ...
            case InternalInterface.MG_ACT_EDT_ENDLINE:
               ...
               ret = postAction(ctrl, internalEventCode);
               break;

            case InternalInterface.MG_ACT_EDT_MARKALL:
               ret = actionMarkAll(ctrl);
               break;
            ...
            case InternalInterface.MG_ACT_MASK_CONTROL_TEXT:
               actionMaskControlText(ctrl, editParams);
               break;
#endif
            case InternalInterface.MG_ACT_EDT_MARKTOBEG:
               ret = actionMarkText(ctrl, 0, editParams.EndPos);
               break;

            case InternalInterface.MG_ACT_EDT_MARKTOEND:
               ret = actionMarkText(ctrl, editParams.StartPos, -1);
               break;

            case InternalInterface.MG_ACT_EDT_MARKPRVCH:
               ret = actionMarkText(ctrl, editParams.StartPos, editParams.EndPos - 1);
               break;
      ...
      /// <param name="keys"></param>
      /// <returns></returns>
      public EditReturnCode PostChar(MgControlBase ctrl, String keys)
      {
         if (ctrl.isModifiable())
         {
      ...
      /// <returns></returns>
      private EditReturnCode postAction(MgControlBase ctrl, int internalEventCode)
      {
         String keys = getKeyFromAct(internalEventCode);
         bool isDeleteOrBackspaceKey = (internalEventCode == InternalInterface.MG_ACT_EDT_DELCURCH || 
      ...
      private void guiSetText(MgControlBase ctrl, String text)
      {
      ...
      private void guiInsertText(MgControlBase ctrl, int startPosition, String textToInsert)
      {
      ...
      private void advancePos(MgControlBase ctrl, PIC pic, int pos)
      {
         int newPos = pos;
#if PocketPC
         if (pic.isAttrBlob() || UtilStrByteMode.isLocaleDefLangJPN())
#else
         if (pic.isAttrBlob())
#endif
            // no mask in blob
         ...
         {
            newPos = convPosBufToMsk(ctrl, newPos);

            while (newPos < pic.getMaskSize() && pic.picIsMask(newPos))
               newPos++;

            newPos = convPosMskToBuf(ctrl, newPos);
            Manager.SetSelection(ctrl, newPos, newPos, newPos);
         }
      }
      ...
      /// <param name = "pic"></param>
      /// <param name = "pos"></param>
      private void retreatPos(MgControlBase ctrl, PIC pic, int pos)
      {
         int newPos = pos;
#if PocketPC
         if (pic.isAttrBlob() || UtilStrByteMode.isLocaleDefLangJPN())
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actionNextChar(MgControlBase ctrl, EditParams editParams)
      {
         // get the new position after advancing
         advancePos(ctrl, ctrl.getPIC(), editParams.EndPos + 1);
         return EditReturnCode.EDIT_CONTINUE;
      }
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actionPrevChar(MgControlBase ctrl, EditParams editParams)
      {
         // get the new position after advancing
         retreatPos(ctrl, ctrl.getPIC(), Math.Max(0, editParams.EndPos - 1));
         return EditReturnCode.EDIT_CONTINUE;
      }
      ...
      /// <param name = "value"></param>
      /// <returns></returns>
      private EditReturnCode actionChar(MgControlBase ctrl, EditParams editParams, String value)
      {
         PIC pic = ctrl.getPIC();
         EditReturnCode rc = EditReturnCode.EDIT_CONTINUE;
         ...
         if (!checkNullDisplayOverride(editParams, ctrl))
            return rc;

         // for rich edit. let the system handle the char.
         if (ctrl.isRichEditControl())
         {
            ctrl.ModifiedByUser = true; // any mod to the ctrl by the user should set this flag.
            // In RichEdit the RichEditHandler does not block the system's action.
            // We canceled the postChar because it uses sendKeys and it is problematic in multi-language.
            // Instead we let the system odo it and we do nothing here.
            // We still need to pass here because we want to raise 'control modified' handlers.
            // postChar(ctrl, evt);
            ctrl.IsNull = false;
            return rc;
         }
         ...
         if (convPosBufToMsk(ctrl, editParams.StartPos) < pic.getMaskSize() || pic.isAttrBlob())
         {
            // get the existing text to work on
            if (editParams.CtrlText == null)
               editParams.CtrlText = new StringBuilder(getCtrlText(ctrl));
            ...
            if (editParams.StartPos == editParams.EndPos && !Manager.GetCurrentRuntimeContext().IsInsertMode())
            {
               ...
               if (ctrl.isMultiline() && editParams.EndPos < editParams.CtrlText.Length &&
                   editParams.CtrlText[editParams.EndPos] == '\n')
                  editParams.EndPos++;
            }
            ...
            if ((editParams.StartPos == editParams.EndPos) || overWriteChar(value[0], ctrl, editParams, true))
               if (charInsert(value[0], ctrl, editParams))
                  // see that caret is positioned correctly
                  advancePos(ctrl, pic, editParams.StartPos);

            // Check if auto skip will be done.
            if (pic.autoSkip())
               rc = checkAutoSkip(editParams, ctrl);
         }
      ...
      /// <returns></returns>
      private bool overWriteChar(char charToWrite, MgControlBase ctrl, EditParams editParams, bool advance)
      {
         PIC pic = ctrl.getPIC();
         int charsToEndOfSeq;
         int endOfSeq;
         int len;
         int startPos = editParams.StartPos;
         int endPos = editParams.EndPos;
         int maskPos = 0;
         ...
         {
            startPos = convPosBufToMsk(ctrl, editParams, startPos);
            endPos = convPosBufToMsk(ctrl, editParams, endPos);
            ...
            if (startPos >= endPos || startPos > convPosBufToMsk(ctrl, editParams, editParams.CtrlText.Length))
            {
            ...
            if (charToWrite > 0)
            {
               ...
               {
                  if (!isValidChar_as400(ctrl, editParams, charToWrite, startPos))
                     return false;
               }
            }

            startPos = convPosMskToBuf(ctrl, editParams, startPos);
            endPos = convPosMskToBuf(ctrl, editParams, endPos);

            // loop on the selected chars (they will be deleted)
            while (startPos < endPos)
            {
               maskPos = convPosBufToMsk(ctrl, editParams, startPos);
               ...
               startPos = convPosMskToBuf(ctrl, editParams, maskPos);
               endOfSeq = convPosMskToBuf(ctrl, editParams, endOfSeq);
               ...
               if (endPos >= endOfSeq)
               {
                  ...
                  while (startPos < endOfSeq)
                  {
                     if (UtilStrByteMode.isLocaleDefLangDBCS() && pic.isAttrAlphaOrDate())
                        isFullWidth = !UtilStrByteMode.isHalfWidth(editParams.CtrlText[startPos]);

                     editParams.CtrlText[startPos] = ' ';

                     if (isFullWidth)
                     {
                        editParams.CtrlText.Insert(startPos, new[] {' '});
                        startPos++;
                        endPos++;
                        endOfSeq++;
                     }
            ...
            int idx = editParams.CtrlText.Length - 1;
            while (idx >= editParams.StartPos && editParams.CtrlText[idx] == ' ' &&
                   !pic.picIsMask(convPosBufToMsk(ctrl, editParams, idx)))
            {
         ...
         guiSetText(ctrl, editParams.CtrlText.ToString());

         // Try to set the caret at the begining of the selected text.
         if (advance)
            advancePos(ctrl, ctrl.getPIC(), editParams.StartPos);
         else
            retreatPos(ctrl, ctrl.getPIC(), editParams.StartPos);
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private bool charInsert(char charToWrite, MgControlBase ctrl, EditParams editParams)
      {
         int charsToEndOfSeq;
         int endOfSeq;
         PIC pic = ctrl.getPIC();
         ...
         int maskPos = 0;
         int maskSize = 0;
         int textLenBeforeChange = editParams.CtrlText.Length;
         ...
         if (pic.isAttrBlob())
#endif
            editParams.CtrlText.Insert(editParams.StartPos, new[] {charToWrite});
         else
         {
            ...
            if (UtilStrByteMode.isLocaleDefLangJPN() || UtilStrByteMode.isLocaleDefLangKOR())
            {
               if (!isValidChar_as400(ctrl, editParams, charToWrite, maskPos))
                  return false;
            }

            if (UtilStrByteMode.isLocaleDefLangJPN() && pic.isAttrNumeric() &&
               UtilStrByteMode.isDigit(charToWrite) && IsFilledNumericDigits(editParams, pic, maskPos)) 
               return false;

            editParams.StartPos = convPosMskToBuf(ctrl, maskPos);
            ...
            else
            {
               ...
               endOfSeq = convPosMskToBuf(ctrl, maskPos + charsToEndOfSeq);
               ...
               if (endOfSeq <= editParams.CtrlText.Length)
               {
                  ...
                  if (!isFullWidth)
                  {
                     if (editParams.CtrlText[endOfSeq - 1] != ' ' || charsToEndOfSeq < 1)
                        return false;
                  }
                  else
                  {
                     if (charsToEndOfSeq < 2 || editParams.CtrlText[endOfSeq - 1] != ' ' ||
                         editParams.CtrlText[endOfSeq - 2] != ' ')
                        return false;
                  }
               }
               ...
               {
                  if (convPosBufToMsk(ctrl, editParams.CtrlText.Length) + (isFullWidth
                                                                              ? 2
                                                                              : 1) > pic.getMaskSize())
                     return false;
         ...
         if (ctrl.isMultiline() && editParams.CtrlText.Length - textLenBeforeChange == 1)
            guiInsertText(ctrl, editParams.StartPos, charToWrite.ToString());
         else
      ...
      /// <returns></returns>
      private EditReturnCode actionDelChar(MgControlBase ctrl, EditParams editParams, bool del)
      {
         int newPos = 0;
         ...
         if (editParams.StartPos == editParams.EndPos)
         {
            ...
            {
               newPos = convPosBufToMsk(ctrl, editParams.StartPos - 1);
#if PocketPC
               ...
               editParams.StartPos = convPosMskToBuf(ctrl, newPos);
               editParams.EndPos = editParams.StartPos + 1;
            }
         }
         ...
         if (editParams.StartPos != editParams.EndPos)
            overWriteChar((char) 0, ctrl, editParams, del);
      ...
      /// <param name = "ctrl"></param>
      /// <returns></returns>
      private EditReturnCode actionCopy(MgControlBase ctrl)
      {
         EditReturnCode retCode = EditReturnCode.EDIT_CONTINUE;

         if (ctrl.isRichEditControl())
         {
            //for a rich edit control, let CLR handle the copy action, because
            //it copies the rtf contents not just the plain text as we do.
            retCode = postAction(ctrl, InternalInterface.MG_ACT_CLIP_COPY);
         }
         else
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actionCut(MgControlBase ctrl, EditParams editParams)
      {
      ...
      /// <returns></returns>
      private EditReturnCode setMaskedText(MgControlBase ctrl, EditParams editParams, String content)
      {
         PIC pic;
         ...
         if (ctrl.isRichEditControl())
         {
            if (editParams.InternalEventCode == InternalInterface.MG_ACT_CLIP_PASTE)
            {
               //if the action is Paste, ignore the clip contents passed to this
               //function because it is the plain text. Instead, let CLR handle
               //this action. It will get the proper formatted text from the clipboard
               //and paste it on the control.
               //This solves one more problem. guiInteractive.clipboardPaste() overwrites 
               //the whole existing text of the control instead of replacing the selected text.
               return postAction(ctrl, InternalInterface.MG_ACT_CLIP_PASTE);
            }
            else
            {
               // let the system handle richedit.
               Commands.clipboardPaste(ctrl, ctrl.getDisplayLine(true));
...
#endif
         {
            int sizeLimit = pic.getMaskSize();
            int currTextLen = editParams.CtrlText.Length;
            int lenToPaste;
            ...
            {
               String strCtrlText = editParams.CtrlText.ToString();
               int clipContentLenB = UtilStrByteMode.lenB(content);

               lenToPaste = sizeLimit -
                            (UtilStrByteMode.lenB(strCtrlText) -
                             UtilStrByteMode.lenB(strCtrlText.Substring(editParams.StartPos,
                                                                        (editParams.EndPos) - (editParams.StartPos))));
            ...
            if (lenToPaste > 0)
            {
               ...
               {
                  String strPasteText = UtilStrByteMode.leftB(content, lenToPaste);
                  newPos = editParams.StartPos + strPasteText.Length;
                  guiSetText(ctrl,
                             editParams.CtrlText.ToString(0, editParams.StartPos) + strPasteText +
                             editParams.CtrlText.ToString(editParams.EndPos,
                                                          editParams.CtrlText.Length - editParams.EndPos).Trim(removeChars));
               }
               else
               {
                  ...
                  guiSetText(ctrl,
                             editParams.CtrlText.ToString(0, editParams.StartPos) + content.Substring(0, lenToPaste) +
                             editParams.CtrlText.ToString(editParams.EndPos,
                                                          editParams.CtrlText.Length - editParams.EndPos).Trim(removeChars));
               }
            ...
            editPasteWithMask(ctrl, editParams, content);
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actionPaste(MgControlBase ctrl, EditParams editParams)
      {
         String clipContent = Manager.ClipboardRead();

         if (clipContent == null || clipContent.Length == 0) 
            return EditReturnCode.EDIT_CONTINUE;

         Manager.Environment.IgnoreReplaceDecimalSeparator = true;
         EditReturnCode retCode = setMaskedText(ctrl, editParams, clipContent);
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actionDrop(MgControlBase ctrl, EditParams editParams)
      {
             ...
             ctrl.isParkable(false, false) && ctrl.isModifiable())
         {
            ...
            Commands.GetSelectionForDroppedControl(ref selectionStart, ref selectionLength);
            if (selectionLength == GuiConstants.REPLACE_ALL_TEXT)
                selectionLength = getCtrlText(ctrl).Length;

            editParams.StartPos = selectionStart;
            editParams.EndPos = selectionLength;
            
            if (!String.IsNullOrEmpty(strData))
               editReturnCode = setMaskedText(ctrl, editParams, strData);
         }
#endif
...
#if PocketPC
      /// <summary> Get the control's accumulated string and set it according to the mask.
      /// Used originally for barcode scanner, where the MgTextBox holds the scanned string
      /// </summary>
      /// <param name="ctrl"></param>
      /// <param name="editParams"></param>
      /// <returns></returns>
      private void actionMaskControlText(MgControlBase ctrl, EditParams editParams)
      {
         String controlAccumulatedText = Commands.accumulatedTextGet(ctrl);
         setMaskedText(ctrl, editParams, controlAccumulatedText);
         return;
      }
#endif
      ...
      private bool editPasteWithMask(MgControlBase ctrl, EditParams editParams, string pasteStr)
      {
         bool checkMask = true; // are we skipping masked chars ?
         PIC pic = ctrl.getPIC();
         int iSkip = 0;
         int posOnPasteStr = 0;
         var pasteStrText = new StringBuilder(pasteStr);
         bool pasteSuccess = true;
         int pasteStrLen = pasteStr.Length;
         int maskPos = convPosBufToMsk(ctrl, editParams.StartPos);

         // does the pasted str start with masked chars ? if so, skip them.
         if (pic.picIsMask(maskPos))
         {
            // check for masked char matching.
            iSkip = edtPasteSkipMaskCheck(pic, maskPos, pasteStrText, 0);
         ...
         if (pasteStrText != null && pasteStrText.Length > 0 && editParams.StartPos != editParams.EndPos)
            if (!overWriteChar(pasteStrText[posOnPasteStr], ctrl, editParams, true))
               pasteSuccess = false;

         if (pasteSuccess)
         {
            char insertChar;
            int maskLen = pic.getMaskLength();
            ...
            while (posOnPasteStr < pasteStrLen && maskPos < maskLen)
            {
               ...
               if (!ctrl.isMultiline() && Char.IsControl(pasteStrText[posOnPasteStr]))
               {
                  posOnPasteStr++;
                  continue;
               }
               ...
               {
                  iSkip = edtPasteSkipMaskCheck(pic, maskPos, pasteStrText, posOnPasteStr);
               ...
               if (maskPos < maskLen)
               {
                  ...
                  // insert the char
                  pasteSuccess = charInsert(insertChar, ctrl, editParams);
      ...
      /// <returns></returns>
      private int edtPasteSkipMaskCheck(PIC pic, int pos, StringBuilder pasteStrText, int posOnPasteStr)
      {
         int iSkip;
      ...
      /// <param name = "editParams"></param>
      /// <returns></returns>
      private EditReturnCode actBegNextLine(MgControlBase ctrl, EditParams editParams)
      {
         if (!ctrl.isModifiable())
            return EditReturnCode.EDIT_CONTINUE;
         ...
         // let the system handle richedit.
         if (ctrl.isRichEditControl())
            return postAction(ctrl, editParams.InternalEventCode);
         ...
         if (editParams.StartPos != editParams.EndPos)
            overWriteChar((char) 0, ctrl, editParams, true);

         // check if adding the eol chars will exceed the mask len. 
         if (ctrl.DataType == StorageAttribute.BLOB ||
             ctrl.getPIC().getMaskLength() >= convPosBufToMsk(ctrl, editParams, editParams.CtrlText.Length) + 2)
         {
            editParams.CtrlText.Insert(editParams.StartPos, OSEnvironment.EolSeq);
            guiSetText(ctrl, editParams.CtrlText.ToString());
            advancePos(ctrl, ctrl.getPIC(), editParams.StartPos + 2);
         }
         return EditReturnCode.EDIT_CONTINUE;
      }
      ...
      /// <param name = "ctrl"></param>
      /// <returns></returns>
      private bool checkNullDisplayOverride(EditParams editParams, MgControlBase ctrl)
      {
         ...
         else
         {
            ...
            if (field != null && field.NullAllowed && ((TaskBase)field.getTask()).IsFieldNull(field) && field.getNullDisplay() != null)
            {
               ...
               if (getCtrlText(ctrl).Equals(ctrl.getField().getNullDisplay()))
                  ...
                  else
                  {
                     ...
                     PIC pic = ctrl.getPIC();
                     String nullVal = DisplayConvertor.Instance.mg2disp(ctrl.getDefaultValueForEdit(), null,
                                                                             pic, false,
      ...
      /// <param name = "ctrl"></param>
      /// <returns></returns>
      private bool nullDisplayIsShown(MgControlBase ctrl)
      {
         Field field = ctrl.getField();
         bool nullDispIsShown = false;
      ...
      private EditReturnCode checkAutoSkip(EditParams editParams, MgControlBase ctrl)
      {
         EditReturnCode rc;
         PIC pic = ctrl.getPIC();
         int maskPos = convPosBufToMsk(ctrl, editParams.StartPos);
         ...
         else
         {
            ...
            {
               if (maskPos - maskCount(0, maskPos, editParams, ctrl) < ctrl.getField().getSize())
            ...
            if (rc != EditReturnCode.EDIT_AUTOSKIP)
            {
               ...
               if (pic.isAttrLogical())
               {
                  ...
                  ValidationDetails vd = ctrl.buildCopyPicture(null,editParams.CtrlText.ToString());
                  vd.evaluate();

                  // if the validation of the fields failed, we cannot skip.
               ...
               {
                  int dummyWhole = 0, dummyDecPos = 0;
                  int dec = edtCountNumericDec(editParams.CtrlText, pic, editParams.StartPos, ref dummyWhole, ref dummyDecPos);
      ...
      private int maskCount(int startPos, int endPos, EditParams editParams,  MgControlBase ctrl)
      {
         int count = 0;
         PIC pic = ctrl.getPIC();
         ...
         startPos = convPosBufToMsk(ctrl, startPos);
         endPos = convPosBufToMsk(ctrl, endPos);
      ...
      private int edtCountNumericDec(StringBuilder ctrlText, PIC pic, int Len, ref int Whole, ref int DecPos)
      {
         int i;
         int Dec;
         char decimalChar = Manager.Environment.GetDecimal();
         ...
         for (i = 0; i < Len; i++)
            if (pic.isNumeric(i))
               if (UtilStrByteMode.isDigit(ctrlText[i]))
                  if (DecPos >= 0)
                     Dec++;
                  else
                     Whole++;
               else if (ctrlText[i] == decimalChar)
                  DecPos = i;
      ...
      /// <param name = "Pos"></param>
      /// <returns> 0 if no decimal point or >0 as the number of digits after the decimal point.</returns>
      private bool IsFilledNumericDigits(EditParams editParams, PIC pic, int Pos)
      {
         bool isFilled = false;
         int Dec = 0;
         int Whole = 0, DecPos = 0;

         if (!pic.isNumeric(Pos))
            return false;

         // Count number of digits and position of decimal point
         Dec = edtCountNumericDec(editParams.CtrlText, pic, editParams.CtrlText.Length, ref Whole, ref DecPos);
      ...
      /// <param name = "ctrl"></param>
      /// <returns></returns>
      private EditReturnCode actionEndField(MgControlBase ctrl)
      {
         int ctrlTextLen = getCtrlText(ctrl).Length;
      ...
      /// <param name = "ctrl"></param>
      /// <returns></returns>
      private EditReturnCode actionMarkAll(MgControlBase ctrl)
      {
      ...
      /// <returns></returns>
      private EditReturnCode actionMarkText(MgControlBase ctrl, int start, int end)
      {
      ...
      /// </returns>
      private int convPosBufToMsk(MgControlBase ctrl, int pos)
      {
         ...
         return pic.isAttrAlphaOrDate()
                   ? UtilStrByteMode.convPos(getCtrlText(ctrl), pic.getMask(), pos, false)
                   : pos;
      }

      private int convPosBufToMsk(MgControlBase ctrl, EditParams editParams, int pos)
      {
         if (pos <= 0 || !UtilStrByteMode.isLocaleDefLangDBCS())
            return pos;
      ...
      ///   ('a', 'b' and 'c' are SBCS, and 'Y' and 'Z' are DBCS)
      /// </returns>
      private int convPosMskToBuf(MgControlBase ctrl, int pos)
      {
         if (pos <= 0 || !UtilStrByteMode.isLocaleDefLangDBCS())
            return pos;
         ...
         PIC pic = ctrl.getPIC();
         return pic.isAttrAlphaOrDate()
                   ? UtilStrByteMode.convPos(pic.getMask(), getCtrlText(ctrl), pos, false)
                   : pos;
      }

      private int convPosMskToBuf(MgControlBase ctrl, EditParams editParams, int pos)
      {
         if (pos <= 0 || !UtilStrByteMode.isLocaleDefLangDBCS())
            return pos;
      ...
      /// <param name = "pos"></param>
      /// <returns></returns>
      private bool isValidChar_as400(MgControlBase ctrl, EditParams editParams, char charToValidate, int pos)
      {
         // check the dbMAGIC_ENV flag (As400Set = N)
         if (Manager.Environment.GetLocalAs400Set() == false)
            return true;

         PIC pic = ctrl.getPIC();
         int firstPos = 0;
         int txtLen = editParams.CtrlText.Length;
         char firstChar = '\x0000';
         ...
         if (txtLen > 0)
         {
            ...
            firstPos = convPosBufToMsk(ctrl, editParams, firstPos);

            if (txtLen > firstPos)
               firstChar = editParams.CtrlText[firstPos];
         }
      ...
      /// <summary>
      ///   Get the text from the ctrl. Richedit will conv the rtf to text before returning it.
      /// </summary>
      /// <returns></returns>
      private string getCtrlText(MgControlBase control)
      {
         string CtrlVal = Manager.GetCtrlVal(control);

         // All we need from the Rtf is the text.
         if (control.isRichEditControl() && Rtf.isRtf(CtrlVal))
         {
            var rtf = new Rtf();
            var outputTxt = new StringBuilder("");
            rtf.toTxt(CtrlVal, outputTxt);
            CtrlVal = outputTxt.ToString();
         }

         return CtrlVal;
      }

      private void sendKoreanImeMessage(MgControlBase ctrl, EditParams editParams, ImeParam im)
      {
         switch (im.Msg)
         {
            case NativeWindowCommon.WM_IME_STARTCOMPOSITION:
               if (editParams.StartPos != editParams.EndPos && !ctrl.isMultiline())
                  actionDelChar(ctrl, editParams, true);
               im.Msg = NativeWindowCommon.MG_IME_STARTCOMPOSITION;
               break;
            case NativeWindowCommon.WM_IME_ENDCOMPOSITION:
               im.Msg = NativeWindowCommon.MG_IME_ENDCOMPOSITION;
               break;
            case NativeWindowCommon.WM_IME_COMPOSITION:
               im.Msg = NativeWindowCommon.MG_IME_COMPOSITION;
               break;
            case NativeWindowCommon.WM_KEYDOWN:
               im.Msg = NativeWindowCommon.MG_KEYDOWN;
               break;
            case NativeWindowCommon.WM_CHAR:
               im.Msg = NativeWindowCommon.MG_CHAR;
               break;
         }
         Manager.SendImeMessage(ctrl, im);
         return;
      }
      ...
      /// <returns>success/fail</returns>
      public bool MarkedTextSet (MgControlBase ctrl, string text)
      {
         bool successful = false;
         MgPoint point = Manager.SelectionGet(ctrl);
         EditParams editParams = new EditParams(point.x, point.y, 0);

         editParams.CtrlText = new StringBuilder(getCtrlText(ctrl));
         successful = editPasteWithMask(ctrl, editParams, text);
         return successful;
      }
   }
}
